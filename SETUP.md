...
- 🎵 노래책: 최근 등록한 곡이 맨 위 (▲▼로 직접 조정 가능) · 아티스트·제목·카테고리·장르·난이도·숙련도·금액(개수)·비고·메모(관리자 전용) · ▲▼ 순서 · 안내 문구
- 📖 안내: 편집 화면 = 게시글과 같은 폭이라 보이는 그대로 저장돼요 · 표 열은 경계선 드래그로 조절 · 👁 미리보기로 최종 화면 확인
- 📖 안내(상세): 규칙/예다의 TMI/가이드/기타 게시판 · 글 검색 · ⊞표 삽입 — 자체 에디터(제목·굵게·기울임·밑줄·취소선·글자색 6·형광펜 3·구분선·인용·접기 토글·이미지·링크), 📌 고정, ✎ 수정, 🔗 글별 공유 링크 복사(`guide/?p=번호`)
- 📮 편지함: 문의 확인/삭제
...
## 6. 배포 확인법 (버전 스탬프)
- 지금 배포된 게 최신인지 확인: 브라우저에서 **F12 → Console** 열면 `LEE YEDA site build 20260806c` 가 찍혀요.
- 관리자 페이지 맨 아래에도 `PRIVATE CONSOLE · BUILD 20260806c` 로 표시돼요.
